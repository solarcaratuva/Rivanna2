...
class SysTimer : private mbed::TimerEvent,
                 private mbed::NonCopyable<SysTimer<Period, IRQ>> {
  public:
    ...
    static_assert(
        std::ratio_divide<period, highres_period>::den == 1,
  ...
  public:
     ...
     * @warning If the ticker tick is already scheduled it needs to be cancelled
     * first!
     */
    virtual void set_wake_time(time_point at) = 0;
     ...
     */
    virtual void cancel_wake() = 0;

    /**
     * Schedule an os tick to fire
     *
     * Ticks will be rescheduled automatically every tick until cancel_tick is
     * called.
     *
     ...
     */
    virtual void acknowledge_tick() = 0;

    /**
     * Prevent any more scheduled ticks from triggering
     *
     * If called from OS tick context, there may be remaining unacknowledged
     * ticks.
     */
    virtual void cancel_tick() = 0;
     ...
     */
    bool ticking() const { return core_util_atomic_load_bool(&_ticking); }
     ...
     * although such a calculation is not atomic if the ticker is currently
     * running.
     *
     * @return number of unacknowledged ticks
     */
    std::chrono::duration<int, period> unacknowledged_ticks() const {
        return std::chrono::duration<int, period>(
            core_util_atomic_load_u32(&_unacknowledged_ticks));
    }
     ...
     * The os_timer should equal this number minus the number of unacknowledged
     * ticks.
     *
     * @return The number of ticks since timer creation.
     */
    virtual time_point get_tick() const = 0;

    /** Update and get the current tick count
     *
     * This is a slow operation that reads the timer and adjusts for elapsed
     * time. Can only be used when the ticker is not running, as there is no IRQ
     * synchronization.
     *
     * This clears the unacknowledged tick counter - the caller is assumed to
     * update their timer based on this return.
     *
     * @return The number of ticks since timer creation.
     */
    virtual time_point update_and_get_tick() = 0;
     ...
     */
    virtual highres_duration get_time_since_tick() const = 0;
     ...
     */
    virtual highres_time_point get_time() const = 0;

    /*protected:
        using highres_duration_u32 = std::chrono::duration<uint32_t,
       highres_period>; void handler() override; void _increment_tick(); void
       _schedule_tick(); duration _elapsed_ticks() const; static void
       _set_irq_pending(); static void _clear_irq_pending(); const
       highres_time_point _epoch; highres_time_point _time; uint64_t _tick;
