...
#if defined MBED_CONF_DRIVERS_SPI_COUNT_MAX &&                                 \
    DEVICE_SPI_COUNT > MBED_CONF_DRIVERS_SPI_COUNT_MAX
...
class SPI {
  ...
  public:
     ...
     *  Not all targets support SSEL, so this cannot be relied on in portable
     * code. Portable code should use the alternative constructor that uses GPIO
     *  for SSEL.
     *
    ...
    SPI(const spi_pinmap_t &&,
        PinName) = delete; // prevent passing of temporary objects
    ...
    /** Assert the Slave Select line, acquiring exclusive access to this SPI
     * bus.
     *
    ...
    /** Deassert the Slave Select line, releasing exclusive access to this SPI
     * bus.
     */
     ...
     *
     * @param tx_buffer The TX buffer with data to be transferred. If NULL is
     * passed, the default SPI value is sent.
     * @param tx_length The length of TX buffer in bytes.
     * @param rx_buffer The RX buffer which is used for received data. If NULL
     * is passed, received data are ignored.
     * @param rx_length The length of RX buffer in bytes.
     * @param callback  The event callback function.
     * @param event     The event mask of events to modify. @see spi_api.h for
     * SPI events.
     *
    ...
    int transfer(const Type *tx_buffer, int tx_length, Type *rx_buffer,
        ...
        if (spi_active(&_peripheral->spi)) {
            return queue_transfer(tx_buffer, tx_length, rx_buffer, rx_length,
                                  sizeof(Type) * 8, callback, event);
        }
        start_transfer(tx_buffer, tx_length, rx_buffer, rx_length,
                       sizeof(Type) * 8, callback, event);
        return 0;
    }

    /** Abort the on-going SPI transfer, and continue with transfers in the
     * queue, if any.
     */
    void abort_transfer();
    ...
    //  * @param tx_length The length of TX buffer in bytes.
    //  * @param rx_buffer The RX buffer which is used for received data. If
    //  NULL is passed,
    ...
    //  */
    // int transfer(const void *tx_buffer, int tx_length, void *rx_buffer, int
    // rx_length, unsigned char bit_width, const event_callback_t &callback, int
    // event);
    ...
    //  * @retval -1 Transfer can't be added because queue is full.
    //  */
    // int queue_transfer(const void *tx_buffer, int tx_length, void *rx_buffer,
    // int rx_length, unsigned char bit_width, const event_callback_t &callback,
    // int event);
    ...
    //  * @param event     The event mask of events to modify.
    //  */
    // void start_transfer(const void *tx_buffer, int tx_length, void
    // *rx_buffer, int rx_length, unsigned char bit_width, const
    // event_callback_t &callback, int event);
...
//     /** Dequeue a transaction and start the transfer if there was one
//     pending.
//      */
...
// #endif // !defined(DOXYGEN_ONLY)
    ...
    // // All members of spi_peripheral_s must be initialized to make the
    // structure
    // // constant-initialized, and hence able to be omitted by the linker,
    // // as SingletonPtr now relies on C++ constant-initialization. (Previously
    // it
    // // worked through C++ zero-initialization). And all the constants should
    // be zero
    // // to ensure it stays in the actual zero-init part of the image if used,
    // avoiding
    ...
    //         SingletonPtr<CircularBuffer<Transaction<SPI>,
    //         TRANSACTION_QUEUE_SIZE_SPI> > transaction_buffer;
    // #endif
    // };
    ...
    //      *  Implemented in order to avoid duplicate locking and boost
    //      performance.
    //      */
    //     void _acquire(void);
    //     void _set_ssel(int);
