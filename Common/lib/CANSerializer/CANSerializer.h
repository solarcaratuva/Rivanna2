...
#define BITMASK(bit_count)                                                     \
    (bit_count >= sizeof(uint64_t) * BITS_PER_BYTE                             \
         ? -1ULL                                                               \
         : (1ULL << bit_count) - 1ULL)
...
#define DESERIALIZE_FIELD_MACRO(r, data, elem)                                 \
    DESERIALIZE_FIELD(BOOST_PP_TUPLE_ELEM(0, elem),                            \
...
#define SERIALIZATION_METHODS(...)                                             \
    void serialize(CANMessage *message) {                                      \
        ...
        BOOST_PP_SEQ_FOR_EACH(SERIALIZE_FIELD_MACRO, (),                       \
    ...
    void deserialize(CANMessage *message) {                              \
        ...
        uint8_t field_start_bit = 0;                                           \
        BOOST_PP_SEQ_FOR_EACH(DESERIALIZE_FIELD_MACRO, (),                     \
                              BOOST_PP_VARIADIC_TO_SEQ(__VA_ARGS__))           \
    }
