...
typedef struct PowerAuxError : CANStruct, rivanna2_power_aux_error_t {
    ...
    void log(int level) {
        log_at_level(
            level,
            "PowerAuxError: fan_error %u, brake_light_error %u, headlight_error %u, bms_strobe_error %u, left_turn_error %u, right_turn_error %u", fan_error, brake_light_error, headlight_error, bms_strobe_error, left_turn_error, right_turn_error);
    }
} PowerAuxError;
