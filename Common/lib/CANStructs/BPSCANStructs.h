...
typedef struct BPSPackInformation : CANStruct, bps_bps_pack_information_t {
    ...
    void log(int level) {
        log_at_level(
            level,
            "BPSPackInformation: pack_voltage %u, pack_current %u, pack_soc %u, discharge_relay %u, charge_relay %u", pack_voltage, pack_current, pack_soc, discharge_relay, charge_relay);
    }
} BPSPackInformation;
...
typedef struct BPSCellVoltage : CANStruct, bps_bps_cell_voltage_t {
    ...
    void log(int level) {
        log_at_level(
            level,
            "BPSCellVoltage: low_cell_voltage %u, low_cell_voltage_id %u, high_cell_voltage %u, high_cell_voltage_id %u", low_cell_voltage, low_cell_voltage_id, high_cell_voltage, high_cell_voltage_id);
    }
} BPSCellVoltage;
...
typedef struct BPSCellTemperature : CANStruct, bps_bps_cell_temperature_t {
    ...
    void log(int level) {
        log_at_level(
            level,
            "BPSCellTemperature: low_temperature %u, low_thermistor_id %u, high_temperature %u, high_thermistor_id %u", low_temperature, low_thermistor_id, high_temperature, high_thermistor_id);
    }
} BPSCellTemperature;
