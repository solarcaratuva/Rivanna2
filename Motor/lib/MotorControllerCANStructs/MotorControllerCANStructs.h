...
public:
    ...
    FrameRequest() {}
    FrameRequest(bool frame0, bool frame1, bool frame2) : frame0(frame0), frame1(frame1), frame2(frame2) {}
...
public:
    ...
    SERIALIZATION_METHODS(
        ...
        (motor_current, 10),
        (fet_temperature, 5),
        (motor_rotating_speed, 12),
        (pwm_duty, 10),
        (advanced_lead_angle, 7)
    );
...
public:
    ...
    SERIALIZATION_METHODS(
        (analog_sensor_error, 1),
        (motor_current_sensor_u_error, 1),
        (motor_current_sensor_w_error, 1),
        (fet_thermistor_error, 1),
        (padding0, 1),
        (battery_voltage_sensor_error, 1),
        (battery_current_sensor_error, 1),
        (battery_current_sensor_adjust_error, 1),
        (motor_current_sensor_adjust_error, 1),
        (accelerator_position_error, 1),
        (padding1, 1),
        (controller_voltage_sensor_error, 1),
        (padding2, 4),
        (power_system_error, 1),
        (overcurrent_error, 1),
        (padding3, 1),
        (overvoltage_error, 1),
        (padding4, 1),
