...
int main() {
    ...
    while(1){
        ...
        SolarVoltage voltage(input_reader.readPanel1Voltage(), input_reader.readPanel2Voltage(), input_reader.readPanel3Voltage(), input_reader.readPanel4Voltage() );  
        vehicle_can_interface.send(&voltage);

        SolarTemp temp(input_reader.readPanel1Temp(), input_reader.readPanel2Temp(), input_reader.readPanel3Temp(), input_reader.readPanel4Temp() );  
        vehicle_can_interface.send(&temp);

        SolarPhoto photo(input_reader.readPanel1Photo(), input_reader.readPanel2Photo(), input_reader.readPanel3Photo(), input_reader.readPanel4Photo() );  
