...
BPSRelayController::BPSRelayController(PinName discharge_en, PinName charge_en, PinName pack_contactor_status)
    : discharge_en(discharge_en), charge_en(charge_en), pack_contactor_status(pack_contactor_status) {
    relay_controller_thread.start(callback(this, &BPSRelayController::relay_controller));
}
...
void BPSRelayController::relay_controller() {
    ...
    pack_contactor_status.rise(callback(this, &BPSRelayController::rise_handler));
    pack_contactor_status.fall(callback(this, &BPSRelayController::fall_handler));

    while (true) {
        log_debug("Waiting for event");
        event_flags.wait_any(BPS_DISCHARGE_ENABLED | BPS_CHARGE_ENABLED | PACK_CONTACTOR_OPENED | BPS_DISCHARGE_DISABLED | BPS_CHARGE_DISABLED, osWaitForever, false);
        log_debug("Iteration of relay_controller with event flags 0x%X", event_flags.get());

        // Disable discharge and charge if BPS disables either or pack contactor opens
        if (event_flags.get() & (PACK_CONTACTOR_OPENED | BPS_DISCHARGE_DISABLED | BPS_CHARGE_DISABLED)) {
            ...
            if (event_flags.get() & PACK_CONTACTOR_OPENED) {
                log_error("Terminated relay_controller_thread and opened discharge and charge relays because pack contactor opened");
            }
            if (event_flags.get() & BPS_DISCHARGE_DISABLED) {
                log_error("Terminated relay_controller_thread and opened discharge and charge relays because BPS disabled discharge");
            }
            if (event_flags.get() & BPS_CHARGE_DISABLED) {
                log_error("Terminated relay_controller_thread and opened discharge and charge relays because BPS disabled charge");
            }
        ...
        if (event_flags.get() & (BPS_DISCHARGE_ENABLED | BPS_CHARGE_ENABLED)) {
            uint32_t flags = event_flags.get() & (BPS_DISCHARGE_ENABLED | BPS_CHARGE_ENABLED);
            ...
            enable_discharge_charge_thread.start(callback(this, &BPSRelayController::enable_discharge_charge));
            log_debug("Started enable_discharge_charge_thread with flags 0x%X", flags);
